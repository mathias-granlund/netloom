...
def render_action_block(title: str, action_def: dict) -> str:
    ...
    if body_fields:
        ...
        field_rows: list[tuple[str, str | None]] = []
        for field in body_fields:
            if not isinstance(field, dict):
                continue
            ...
            if row is None:
                continue
            name, detail = row
            detail = f"{detail} ({requirement})" if detail else f"({requirement})"
            field_rows.append((name, detail))
        lines.extend(_format_aligned_rows(field_rows))
