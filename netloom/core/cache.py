...
class InteractiveCatalogSpec:
    cache_filename: str
    index_filename: str
    catalog_versions: tuple[int, ...]
    index_version: int


_INTERACTIVE_CATALOG_SPECS = {
    "clearpass": InteractiveCatalogSpec(
...
def _catalog_spec(plugin_name: str | None) -> InteractiveCatalogSpec | None:
    normalized = _normalize_plugin_name(plugin_name)
...
def _valid_catalog(
    data: dict[str, Any] | None, spec: InteractiveCatalogSpec
) -> dict[str, Any] | None:
    if (
...
def _cache_file_path(spec: InteractiveCatalogSpec, settings: Settings) -> Path:
    return settings.paths.cache_dir / spec.cache_filename


def _index_file_path(spec: InteractiveCatalogSpec, settings: Settings) -> Path:
